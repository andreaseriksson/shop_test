  ...
  def inputs(%Schema{} = schema) do
    Enum.map(schema.attrs, fn
      ...
      {key, :decimal} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, step: "any" %>), error(key)}
      {key, :boolean} ->
        {checkbox_label(key), ~s(<%= checkbox f, #{inspect(key)}, class: "tag-checkbox" %>), error(key)}
      {key, :text} ->
        {label(key), ~s(<%= textarea f, #{inspect(key)}, class: "tag-input" %>), error(key)}
      {key, :date} ->
      ...
      {key, _}  ->
        {label(key), ~s(<%= text_input f, #{inspect(key)}, class: "tag-input" %>), error(key)}
    end)
  end

  defp label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "tag-label" %>)
  end

  defp checkbox_label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "tag-label ml-1 inline" %>)
  end
